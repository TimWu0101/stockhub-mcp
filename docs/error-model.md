# market-data-mcp 错误模型与容错设计

本文定义 `market-data-mcp` 的统一错误模型、部分成功机制、质量标记和核心容错策略，用于保证主流程可闭环，并在多数据源、跨市场、AI 高频调用场景下保持可解释性和稳定性。
...
`market-data-mcp` 真正要做到“闭环”，不只是能拿到数据，还要做到：
