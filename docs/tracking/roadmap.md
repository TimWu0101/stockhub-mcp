...
### 目标
已交付：基本面/估值/分红/持仓/期权/指数对比。新闻层待 V0.4。
...
已交付：`get_quick_analysis`、`get_valuation_percentile`、`get_risk_metrics`（波动率/回撤/夏普/VaR/Beta）。

待开发：`analyze_portfolio_exposure`、`get_correlation_matrix` 等组合分析工具。
...
|---|---|---|
| V0.1 行情基础版 | 10 | ✅ 全通过 |
| V0.2 中国市场增强版 | 17 | 🚧 已实现（交易日验证中） |
| V0.3 研究与上下文版 | 11 | 🚧 已实现 |
| V0.4 组合与风控 | +5 | ✅ 已完成 |
| **当前总计** | **43** | |

**PyPI 已发布**：https://pypi.org/project/stockhub-mcp/
