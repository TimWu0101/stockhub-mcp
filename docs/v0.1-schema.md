# market-data-mcp V0.1 Schema 定义

> 本文定义 `market-data-mcp` V0.1 所有工具的入参、返回结构、通用字段、枚举值和质量标记。
> 后续所有工具实现必须以此为基准。
...
| `policy` | `string` | 命中的策略名，见 `docs/cache-strategy.md` |
...
详细定义见 `docs/error-model.md`，以下为最常用项：
